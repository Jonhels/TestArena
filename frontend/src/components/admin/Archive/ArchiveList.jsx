...
import "../Inquiry/InquiryList/InquiryList.css";
...
function ArchiveList({
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      const dropdowns = document.querySelectorAll(
        ".dropdown-menu, .menu-trigger, .arrow-icon"
      );
  ...
  const filterInquiries = () => {
    ...
    if (selectedResponsible !== "Alle") {
      result = result.filter(
        (inq) => (inq.assignedTo?.name || "Ingen") === selectedResponsible
      );
    }
  ...
  const uniqueResponsibles = [
    "Alle",
    ...Array.from(new Set(inquiries.map((i) => i.assignedTo?.name || "Ingen"))),
  ];
  ...
  const handleDelete = async (e, inquiryId) => {
    ...
    const confirmDelete = window.confirm(
      "Er du sikker på at du vil slette denne henvendelsen?"
    );
  ...
  const renderPaginationInfo = () => (
    <span className="result-info">
      ...
      {Math.min(currentPage * ITEMS_PER_PAGE, filteredInquiries.length)} av{" "}
      {filteredInquiries.length} henvendelser
    </span>
  );
                ...
                <div onClick={(e) => e.stopPropagation()}>
                  ...
                  {openDropdownId === inq._id && (
                    <div className="dropdown-menu">
                      <p
                        className="handlearchive"
                        onClick={(e) => handleRestore(e, inq._id)}
                      >
                        Gjenopprette
                      </p>
                      <p
                        className="deleteArchived"
